...
> 대상: `C:\projects\matrox-frame-grabber` (소스·테스트 합 23,668 LOC)
> 최초 작성: 2026-08-18, 커밋 `07e9cbc`
> 갱신: **2026-09-11.** 녹화 계약 · 상시 녹화 · 사건 증거 · 저장 정책이 들어온 13커밋만큼
> 뒤처져 있었다. 없어진 파일 하나를 아직 설명하고 있었고(`Mil/RecordingSession.cs`),
> `CameraChannel`을 2,096줄이라고 적고 있었다(실제 3,542). 아래 §2.8·2.15·2.17을 고치고
> §2.18~2.21을 새로 썼으며, **§9 부정 결과**를 추가했다.

## 이 문서의 표기 규율

근거의 종류를 섞지 않는다. `C:\projects\cs` 분석 문서에서 가져온 규율이고, 그쪽에서
초판이 단일 실행값을 반복 측정값처럼 적어 2.56배를 2.2배로 정정한 일이 있었다.
이 저장소에서도 같은 일이 있었다 — 증거 링 버퍼의 "유실 0"을 **1채널 측정**으로 보고했다가
3채널에서 뒤집혔다.

- **[측정]** — 이 장비에서 직접 실행해 확인. 조건(채널 수·레이트·시간)을 함께 적는다.
- **[그래프]** — 바이너리나 파일에서 직접 추출.
- **[추정]** — 코드 근거는 있으나 실행으로 확인하지 못함.

**측정값에는 채널 수를 반드시 붙인다.** 이 앱에서 1채널과 3채널은 다른 시스템이다 —
취득 대역폭에 공유 천장이 있어서다(§8).
...
용도가 정해진 뒤로 **AVN 화면 이상 검지** 쪽 코드가 붙었고, 지금은 그것이 앱의 목적이다
(DRTMS — Display Product Reliability Test Monitoring System). 분석 ROI, 밝기 측정, PWM 노출 스윕,
타일 검지기가 여기 속하며 대부분 `Infrastructure`에 MIL 무의존으로 있다.
**검지는 완성되어 그랩 경로에서 돌고 있다** — Dropout 하나가 구현돼 있고 나머지 여섯 종류는
카탈로그가 미구현이라고 말한다.
...
| tier | 무엇 | 언제 도나 |
|---|---|---|
| **세션** | 5분 `.ts` 세그먼트, H.264, NAS로 옮겨진다 | `● Rec` |
| **사건** | 2초 세그먼트 링 버퍼 + 확정 시 `-c copy` 클립 + 무손실 정지화면 4장 | 그랩과 함께 |
| **증거** | 무압축 프레임을 RAM 링 버퍼에 들고, 확정된 구간만 Ut Video로 쓴다 | `EvidenceSeconds > 0` |

**세션 파일과 사건 클립은 수명이 달라 프로세스 하나에 출력 둘로는 안 된다.** 그래서 싱크가 둘이고,
그 둘이 MIL에서 프레임을 각자 꺼내다가 유실을 냈다 — 지금은 `FrameExtractor`가 한 번 꺼내
`SharedFramePool`이 참조를 센다(§2.18).
...
### 2.7 `Mil/CameraChannel.cs` (3,542줄) — 앱의 심장

> 2026-09-01에 2,096줄이라고 적혀 있었다. 검지·녹화 tier 셋·증거를 붙이면서 그만큼 늘었고,
> **MIL과 검출·녹화를 잇는 코드를 `Infrastructure`로 내리는 것이 다음 정리 대상이다.** 이 클래스는 모델이자
> 뷰모델이며(pane의 `DataContext`), 그것이 커지는 이유의 절반이다.
...
### 2.8 녹화 계약 — `Mil/Video/` (2026-09-10 교체)

> **이 절은 `Mil/RecordingSession.cs`를 설명하고 있었다. 그 파일은 없다.** 녹화가
> `IVideoSink` 뒤로 들어가면서 삭제됐고, 문서만 열흘 남아 있었다.

`VideoSinkFactory`가 시작할 때 백엔드를 고르고 **그 이유를 로그와 Rec 버튼 툴팁에 남긴다.**

| 파일 | 줄 | 역할 |
|---|---|---|
| `IVideoSink.cs` | 116 | 계약. `Feed`가 **`byte[]`가 아니라 `MIL_ID`** 를 받는다 |
| `FfmpegVideoSink.cs` | 373 | 유일하게 도는 백엔드. stdin 파이프로 플래나 `gbrp` |
| `MilSeqVideoSink.cs` | 141 | **이 장비에서 한 번도 실행된 적 없는 골격** |
| `VideoSinkFactory.cs` | 101 | 고르고, 왜인지 말한다 |
| `FrameExtractor.cs` | 250 | MIL→호스트 읽기를 **한 번만** 한다 (§2.18) |
| `IHostFrameSink.cs` | 44 | 호스트 바이트를 받는 쪽의 계약 |

**`Feed`가 `MIL_ID`를 받는 것이 계약의 핵심이다.** ffmpeg 경로는 프레임을 호스트로 읽어내야
하지만(**[측정]** 462 µs) `MseqFeed`는 버퍼를 그대로 받는다 — 바이트로 받는 계약은 두 번째
백엔드에 첫 번째의 비용을 강요한다. 호스트 바이트를 받는 것은 그래서 `IVideoSink`가 아니라
**별도의 `IHostFrameSink`** 다. 쓰지도 않을 메서드를 납품 계약에 넣지 않는다.

**컬러 프레임 추출 함정은 그대로 유효하다**(옛 §2.8의 핵심이었고 지금은 `FrameExtractor`에 있다):

```
MbufGetColor 의 패킹 경로는 이 버퍼에서 hang 하거나 0을 반환하고,
평범한 MbufGet 은 band 0 만 준다.
→ planar 3-band 버퍼를 만들고 MbufChildColor 로 밴드별 자식을 만든 뒤
  밴드마다 MbufGet 해서 planar `gbrp` 로 넘긴다.   순서는 [G][B][R].
  자식은 부모보다 먼저 해제한다.
```

**레이트도 해상도도 설정이 아니다.** 둘 다 취득에서 나온다. 설정으로 있었을 때 30을 입력하면
파일은 31.079을 선언했고, 해상도 프리셋은 1024×772 앞에서 1080p가 아무 일도 하지 않고 720p가
0.932배였다 — **[측정]**. 계약에는 배율과 everyNth가 남아 있지만 **앱은 언제나 1.0과 매
프레임을 넘긴다.**

`tools/MilVideoSink/`(295줄)가 MIL 인코딩을 개발·계측할 독립 하네스이며 **도메인 로직 0으로
납품사에 넘기는 슬라이스**다.
...
### 2.15 타일 검지기 (2026-08 추가, **2026-09-03 완성**)

> "미완. MIL 접착부와 분석 스레드는 아직 없다"고 적혀 있었다. **둘 다 있고 돌고 있다** —
> `Mil/TileReducer.cs`(318줄)가 그랩 훅에서 프레임을 타일로 줄이고, 분석은 별도 스레드가
> 아니라 **훅 안에서** 돈다(**[측정]** reduce 평균 200 µs / 프레임 주기 8333 µs).
> 판정은 `RefreshStats()` 틱이 아니라 프레임마다 일어난다 — 사건형은 프레임 하나로도
> 확정되어야 하기 때문이다.

`AnomalyDetector`(921줄)가 지속형과 사건형을 **한 상태머신으로 묶지 않는다.** 확정 방식이
다르다 — 지속형은 히스테리시스로 콘텐츠 변화와 가르고, 사건형은 프레임 하나로 확정된다.

**종류는 일곱이고 구현은 하나다.** `AnomalyCatalog.Implemented`가 Dropout만 참을 돌려주고
설정 창이 나머지를 회색 "미구현"으로 그린다 — 숨기지 않는다. 감시할 종류는
`AnomalyKindSet`으로 **앱 전체 설정**에 하나만 저장되고(이름 배열), 채널의
`KindSettings.Enabled`는 그 정책의 사본이라 `[JsonIgnore]`다. **키가 없는 것과 빈 배열은
다르다** — 없으면 기본값(Dropout), 있고 비어 있으면 정말 아무것도 감시하지 않는다.
후자를 기본값으로 접었더니 체크를 모두 풀면 다음 시작에서 되살아났다.
...
DetectionSettingsTests   AnomalyKindSetTests      ClipSchedulerTests
FfmpegArgsTests          VideoRatePolicyTests     VideoEncodingTests
SegmentRingTests         SharedFramePoolTests     RecordingRecordTests
StoragePolicyTests       HostMemoryTests
EvidenceRingTests        EvidenceRingLookupTests  EvidenceGateTests
RecordingRecordFramesTests
```

**녹화 결함 두 건이 `FfmpegArgs`와 `VideoRatePolicy`에 살았다.** 그것이 명령줄과 레이트
산술을 `Infrastructure/Video/`로 내린 이유다 — MIL 없이 검증되는 자리에 있어야 하는 것은
MIL을 부르지 않는 산술 전부다.
...
### 2.18 프레임을 한 번만 꺼낸다 — `FrameExtractor` + `SharedFramePool` (2026-09-10)

싱크 둘이 각자 MIL에서 읽어내자 유실이 났다. 3채널 · 1024×772 · 124.3 fps로 5분씩 재고,
고친 뒤 다시 5분을 쟀다 — **[측정]**.
...
추출 최대가 프레임 주기(8043 µs)를 넘으면 보드가 프레임을 버린다. 핵심은 두 번째 싱크를 붙였을 때
각 추출이 2배가 아니라 **4배** 느려졌다는 것 — 단순 중복이 아니라 메모리 경합이다(프레임당 호스트
읽기 2 × 2.26 MiB × 124.3 × 3채널 = 1.68 GB/s, 취득 DMA가 같은 경로에 0.85 GB/s). 한 번으로
줄이자 **남은 한 번도 2~3배 빨라졌다.**

**`SharedFramePool`의 규약은 순서가 전부다.** `Take`가 호출자에게 hold 1을 주고, 프레임을 받는
싱크가 **큐에 넣기 전에** hold를 더하고, 모두가 정확히 한 번 `Release`한다. 순서가 반대면
writer 하나가 먼저 끝났을 때 다른 싱크가 읽는 중에 버퍼가 재사용된다. 그 계수는
`Infrastructure`에 있고 테스트가 지킨다.

검증 — **[측정]**: 파일 셋을 전부 디코딩해 경고 0에 프레임 수가 먹인 수와 정확히 일치
(37303/37310/37314), `pool dry 0회`, `still out 0`. 그리고 **무손실 20초 녹화의 2487프레임이 모두
서로 다른 해시**였다 — 센서 잡음 때문에 실제 프레임 둘이 같을 수 없으므로 중복이 없다는 것은
버퍼가 두 번 나가지 않았다는 뜻이다.

---

### 2.19 상시 녹화와 저장 정책 — `Infrastructure/Video/` (2026-09-11)

| 파일 | 줄 | 역할 |
|---|---|---|
| `VideoEncoding.cs` | 213 | `H264`/`Lossless`/`Uncompressed` + 컨테이너 함정표 |
| `RecordingRecord.cs` | 257 | 산출물 옆 사이드카 JSON. 손으로 쓴다(불변 문화권) |
| `StoragePolicy.cs` / `StorageWarden.cs` | 137 / 186 | 로컬 스테이징을 비우는 정책과 집행 |
| `SegmentRing.cs` | 294 | 사건 tier의 세그먼트 목록·수명 |
| `ClipExtractor.cs` | 141 | `-c copy` 잘라내기 |

**세션 레이트는 파일에 적히지 않는다.** 취득 레이트의 약수가 되고 파일은 **그 결과**를 선언한다 —
30 요청 → 124.316의 4프레임마다 → **31.079** 선언. 노출 100 ms(상한 10 fps)면 약수가 1이고 파일은
10 fps다.

**파이프에 넣는 것과 선언하는 것이 같아야 한다** — **[측정]**: 4프레임마다 넘기면서 입력을
124.316이라고 선언하면 ffmpeg이 다시 3/4을 버려 **100프레임 중 27개만** 남는다. 단일 출력이
N프레임마다 받을 때는 `-framerate`도 파일 레이트다.

**컨테이너는 정전 내성으로 고른다** — **[측정]**, ffmpeg을 12초 지점에서 강제 종료:
MP4 7.08 MB에 **0프레임**, MPEG-TS **1480프레임 / 11.93초 정상**, Matroska는 프레임은 회수되나
duration 없음. 잘린 TS는 `-c copy`로 0.07초에 MP4가 된다.

**세그먼트 경계는 비용이 없다** — **[측정]**: 10초 세그먼트(180초에 채널당 18개 경계) 대 단일 파일,
유실 둘 다 0, 경계 손실 0(세그먼트 18개 합계 22382프레임 = 먹인 22382).

**증거는 어느 정책도 지우지 않고, 그것을 파일명이 아니라 폴더로 보장한다.** 삭제는 `continuous/`
안만 보고, 그 안에서도 미디어 파일만 후보다 — 사이드카와 세그먼트 목록은 몇 KB짜리 *기록*이라
영상이 사라져도 남는다. 2분보다 어린 파일은 건드리지 않는다(쓰고 있는 세그먼트일 수 있다).

**사이드카의 감사 한 줄** — 파일이 선언한 레이트 대 보드가 전달한 레이트. 둘은 어긋날 이유가
없고(**[측정]** 잔차 +0.014%, 카메라 발진기) 유실이 있으면 비례해서 벌어진다. 증거에는
`first_frame`/`last_frame`이 함께 들어간다 — 그것이 없으면 증거 파일의 N번째 프레임이 검출기의
몇 번인지 말할 수 없다.

---

### 2.20 무압축 증거 링 버퍼 — `EvidenceRing` + `EvidenceGate` (2026-09-11)

클립으로는 답할 수 없는 질문이 있다. 클립은 H.264 링 버퍼에서 잘라내므로 **그 파일로 검출기를 다시
돌려도 보고된 편차가 재현되지 않는다.** 그런데 무손실 링을 상시 쓰면 채널당 295 MB/s, 3채널
884 MB/s = **하루 76 TB**(1 TB TLC SSD 총 수명 약 750 TB)다. 그래서 호스트 RAM이다.

```
프레임당       1024 × 772 × 3 = 2,371,584 B = 2.37 MB (= 2.26 MiB)
저장 구간(초)  전후 N × 2 + 사건 최대 길이
링 버퍼(초)    저장 구간 ÷ UsableFraction(0.66)
용량          ceil(링 버퍼초 × fps) × 프레임당 바이트

±3초 · 119.990 fps → 1456프레임 = 3.45 GB/카메라, 3채널 10.36 GB
```

**항이 셋이고 하나라도 빼면 프레임을 잃는다.** ②(사건 자체의 길이)와 ③(쓰는 동안의 여유)를 처음에
빼먹었더니 **8건 중 2건이 `1 LOST to the ring`** 을 찍었다 — **[측정]** 이 산술의 출처다.

**세 가지가 실측에서 나왔다.**

1. **링 버퍼는 할당할 때 미리 만져 둬야 한다.** `new byte[]`는 demand-zero 페이지라 처음 한 바퀴
   도는 동안 4 KB마다 페이지 폴트를 취득 옆에서 낸다. **[측정]** 3채널 ±2초 60초:
   미리 안 만지면 32/25/28, 만지면 10/10/10, 120초로 늘리면 5/1/1(남은 것도 시작 과도).
2. **스케줄러가 병합하므로 요청 구간이 버퍼보다 넓어질 수 있다**(**[측정]** 22건 병합 = 18.8초).
   덤프는 `UsableSpanSec`으로 잘라 최근 쪽을 남긴다.
3. **버퍼의 배열을 인코더 큐에 넣으면 안 된다.** `WriteFrame`은 큐에 넣고 바로 돌아오고 파이프
   쓰기는 최대 8프레임 뒤다 — 그 사이 버퍼가 슬롯을 덮는다. 동기 `WriteFrameNow`로 바꿨고,
   큐 경로가 **받은 것보다 3프레임 적게** 넣고 있던 것도 함께 잡혔다.

**락은 인덱스 쓰기에만 잡는다.** 처음에는 `Add`가 2.37 MB memcpy를 락 안에서 했고 `SlotOf`가
1456칸 선형 탐색이었다 — 덤프가 프레임마다 두 번 부르므로 초당 약 200만 비교가 취득 스레드가
필요한 그 락 안에서 돌았다. 슬롯 예약 → 락 밖 복사 → 인덱스 발행으로 바꾸고, 프레임 번호가
오름차순이고 쓰기가 링을 순서대로 도는 성질을 써서 **회전된 순서 위의 이진 탐색**(11 비교)으로
바꿨다. **[측정]** 같은 폭주에서 유실 64/62/49 → **6/4/4**, `read mean` 2216~2337 → 1493~1501 µs.

**덤프는 채널을 가로질러 직렬화된다**(`EvidenceGate`, 전역 허가 1개). 덤프 하나가 660 MB이고 셋이
겹치면 디스크에 2 GB/s를 요구한다 — **[측정]** 그때 레이트가 120 → 57~89 fps로 내려앉았다.
게이트는 워커 스레드에서 잡고(틱에서 잡으면 UI가 멈춘다) **저장 구간은 대기가 끝난 뒤에
해석한다**(큐에 서 있는 동안에도 버퍼는 찬다).

**`HostMemory.Fits`가 할당 전에 물어본다.** Windows는 이 크기를 거절하지 않고 페이징으로 받아
주는데, 취득 옆의 페이징이 곧 프레임 유실이다. 여유의 60%를 넘으면 링 없이 시작하고 그렇게 적는다.

**검증 — 두 독립 경로가 같은 바이트를 낸다** **[측정]**: 증거 파일에서 디코드한 프레임 5000이 같은
프레임의 무손실 PNG와 **바이트 단위로 동일**했다. MIL 버퍼 → `MbufExport` 경로와 RAM 버퍼 →
ffmpeg 경로가 서로 독립인데도 같은 해시다.

---

### 2.21 운전점 — 노출이 레이트를 정한다 (2026-09-11)

이 카메라는 카메라 쪽 ROI를 지원하지 않고(쓰기를 받아들이고 무시한다) `AcquisitionFrameRate`의
상한이 `ExposureTime`에 종속된다. 따라서 **페이로드를 건드리지 않고 레이트를 옮기는 레버는
노출뿐**이고, `--exposure`가 그것을 세 채널에 한 번에 건다.

**[측정]** 노출 8289 µs → **119.990 fps**(세 채널 동일, 목표에서 0.008%). 8000 µs는 124.316이다.

**120이 124.3보다 확연히 낫다.** 같은 "한 대에서만 사건" 프로토콜이 124.3에서는 8/7/8에 덤프
22건이었고 120에서는 **3/0/2에 덤프 33건**이다 — 3.5%의 레이트 차이가 이만큼 벌어지는 것은 이
지점이 꼬리 지연의 무릎이라는 뜻이다. **상시 운전은 120으로 잡는다.**
...
**세 번째가 될 수 있는 것:** 학습 판독기를 들이면 ONNX Runtime(win-x64 네이티브 15.4 MB)이
붙는다. 가중치 파일은 ffmpeg처럼 런타임 탐색이고 저장소에 넣지 않는다. 붙이는 자리와 **빼는
비용**은 `docs/adr/0002-screen-inspector-as-a-removable-slice.md`에 있다 — 아직 코드는 0줄이다.
...
## 9. 부정 결과 — 하지 말 것 (측정상 이득 없음)

`C:\projects\cs` 분석 문서에서 가져온 절이다. 시도했다가 버린 것이 흔적 없이 사라지면 다음 사람이
같은 것을 다시 시도한다.

- **표시 경로 최적화로 취득 fps를 올리려는 것** — **[측정]** 확인. 취득 대역폭에는 공유 천장이
  있고(§8) **프레임당 페이로드만이 레버다.** 표시를 아무리 빠르게 해도 취득 fps는 늘지 않는다.
- **증거 링 버퍼의 프레임당 memcpy를 없애려는 것** — 사건이 없을 때 3채널 120 fps에서 유실
  **0/0/0**이다(**[측정]** 120초). 프레임당 복사는 공짜다. 비쌌던 것은 락이었고 그건 고쳤다.
- **추론을 위해 CPU를 아끼려는 것** — **[측정]** 코어를 다 태우며 초당 212회 추론해도 취득 유실
  0/0/0, UI 지연 중앙값 0.12 ms. **이 시스템은 CPU가 아니라 대역폭에 묶여 있다** — 덤프는 660 MB를
  취득 DMA와 같은 경로로 밀어 프레임을 잃고, 추론은 246 KB 텐서 위의 계산이라 캐시에 들어간다.
  **무엇을 추가할지 판단할 때의 기준이다.**
- **`IntraOpNumThreads`를 손으로 맞추려는 것**(ONNX를 들이는 경우) — **[측정]** ORT 기본값이
  8스레드 수동 설정과 같다(2.707 대 2.696 ms). 모델을 바꾸면 다시 재야 하지만, 지금 모델에서는
  건드릴 이유가 없다.
- **전처리를 SIMD화하거나 `Marshal.Copy`를 없애려는 것**(같은 경우) — cs 문서 §8이 인터리브 반복
  측정으로 전처리+디코드 전체가 추론의 2% 미만이고 **실행 간 편차 이하라 측정 자체가 불가능**함을
  확인했다.
- **총량만 낮춰 NAS 전송 부하를 줄이려는 것** — **[측정]** 꼬리 지연은 평균이 아니라 **버스트
  강도**에 반응한다. 총량을 30 MB/s로 낮추고 버스트를 그대로 두면 효과가 없었다. 조각 단위로
  사이를 띄우는 것이 듣는다.

### 실측이 뒤집은 것 (같은 실수를 반복하지 않기 위해)

- **"증거 링 버퍼 유실 0"은 1채널 측정이었다.** 3채널에서 32/25/28이었다. **측정값에 채널 수를
  반드시 붙인다.**
- **"남은 유실은 메모리 대역폭"도 틀렸다.** 대부분 락 경합이었고, 고쳐서 64/62/49 → 6/4/4가 됐다.
- **"3채널 매 프레임 추론은 예산 100%라 불가"의 결론은 맞고 이유가 틀렸다.** 추론은 취득 예산
  안에서 돌지 않는다 — 다른 코어에서 돈다. 진짜 이유는 처리량(212회/초 대 필요 360회/초)이다.
