...
tests/                         MatroxFrameGrabber.Tests (511개). csproj가 `Infrastructure/**`를
...
보드가 전달한 레이트.** 그리고 증거에는 **`first_frame` / `last_frame`** 이 함께 들어간다.
파일은 프레임의 나열일 뿐이고 검출기는 프레임 번호로 보고하므로, 둘을 잇는 오프셋이 없으면
**증거 파일의 N번째 프레임이 무엇인지 말할 수 없다** — 프레임 5000을 검증할 때 쓴 4503은 로그에만
있었고 로그는 순환된다. 관계는 `파일 내 인덱스 = 프레임번호 − first_frame`이고,
`frames_dropped == 0`일 때 정확하다. 그 값이 바로 옆 칸에 있다. 둘은 어긋날 이유가 없고(실측 잔차 +0.014%, 카메라 발진기), 유실이 있으면
...
| 세 대 동시 사건 (락까지 고친 뒤) | **6 / 4 / 4** | 30초에 24건, LOST 0 |
...
**그리고 남은 64/62/49의 대부분은 락 경합이었다 — 고쳐서 6/4/4가 됐다.**

처음에는 메모리 대역폭으로 읽었다. 같은 사건 폭주에서 증거만 끄면 0/0/0이고(`read mean`
364~516 µs) 켜면 2216~2337 µs로 6배가 되니, 덤프 자체의 몫인 것은 맞았다. 틀린 것은 **어느**
몫인지였다.

`EvidenceRing`이 두 가지를 잘못하고 있었다.

- **`Add`가 2.37 MB memcpy를 락 안에서 했다.** 취득 스레드가 초당 120번 그 락을 그만큼 붙잡고
  있었다.
- **`SlotOf`가 선형 탐색이었다.** 1456칸을 훑고, 덤프가 프레임마다 두 번 부른다(`Peek` +
  `StillHolds`). 초당 약 700프레임이면 **200만 비교/초가 같은 락 안**이다.

고친 것은 슬롯을 예약해 두고 **복사는 락 밖에서** 한 뒤 인덱스만 발행하는 것, 그리고
프레임 번호가 오름차순이고 쓰기가 링을 순서대로 도는 성질을 써서 **회전된 순서 위의 이진
탐색**으로 바꾼 것이다(11 비교). 복사 중인 슬롯은 `-1`로 표시해 두므로 그 사이의 독자는
`StillHolds`가 원래 주던 답("링이 추월했다")을 더 일찍 받는다.

실측(같은 조건, 3채널 30초 세 대 모두 폭주): 유실 **64/62/49 → 6/4/4**,
`read mean` **2216~2337 → 1493~1501 µs**, 덤프 24건 LOST 0, 레이트 120.0 유지.

cs 문서 §6.6("락 안에서 921 KB 복사")을 우리 코드에 대입해 찾은 것이다. 남은 6/4/4는 이제
대역폭으로 보이고, 그 빈도(30초에 24건)는 운전 조건이 아니다 — 그렇게 자주 사건이 나면 링
버퍼가 애초에 구간을 담지 못한다.
...
## 임계값은 1시간 정숙 실행에서 채택했다 (2026-09-11)

5분 실행으로는 안 됐다. 예산을 **11.8/시간까지만 분해**하고, 세 채널이 *같은 프레임 번호*로 사람
팔을 봤다. 한 시간을 돌려 처음으로 채택 가능한 값이 나왔다.

**실행(120 fps · 3채널 · 각 432,080프레임):** 사건 0, onset-spread 기각 0, coherence 단독 기각 0,
**유실 0**, 그리고 `this run resolves 1/hour` — 목표 예산을 분해할 수 있는 첫 실행이다.

| | 잡음 바닥 | 히스토그램 제안 | **채택 (2×바닥)** | 이전 |
|---|---|---|---|---|
| ch0 | 0.0289 | 0.030 (1.04×) | **0.058** | 0.05 |
| ch1 | 0.0318 | 0.033 (1.04×) | **0.064** | 0.15 (4.7×) |
| ch2 | 0.0210 | 0.022 (1.05×) | **0.042** | 0.05 |

**제안값을 쓰지 않았다.** 히스토그램 제안은 예산을 정확히 맞추는 지점이라 통계적으로는 맞지만
**관측된 최대 잡음 바로 위 3.5%**다. 이 저장소가 depth 0.05를 정할 때 쓴 여유는 2배였고, 그것을
유지했다. 정작 중요한 발견은 **ch1이 바닥의 4.7배였다는 것** — 10% 깊이의 실제 하강을 놓치고 있었다.
출처는 `CalibratedAt` / `CalibrationFrames` / `CalibrationFloor`에 함께 저장된다.

**바닥이 5분 때보다 3배 올랐다**(0.0091 → 0.0289). 패널이 그때는 정지된 어두운 화면이었고 지금은
콘텐츠를 띄우고 있다 — 신호가 크면 잡음도 크다. **짧은 실행의 바닥을 길게 외삽하면 안 된다**는
뜻이기도 하다: 채택 후 1분 실행의 바닥은 0.0079였고, 그건 드문 꼬리를 못 본 값이다.

**Blackout은 6 / 12 / 4를 유지했다.** 작동 패널이 **118.9 / 87.8 / 36.2 luma**로 바닥을 쳤으므로
6.0은 각각 바닥의 5.0% / 6.8% / 16.6%다. 여기서 `Propose()`의 휴리스틱이 틀린 것이 드러났다 —
바닥의 절반을 제안했는데 **59 luma는 사라진 패널이 아니라 절반 밝은 패널**이고 그건 Dropout이 볼
일이다. 지금은 1/4에 상한 10 luma로 고쳤다(제안 10 / 10 / 9.1). 그래도 현재 6이 더 보수적이라 그대로
뒀다.

**이 값들이 막는 것은 오검출뿐이다.** 진짜 blackout이 몇 luma인지는 **한 번도 측정한 적이 없다** —
카메라를 가려 봐야 알 수 있고, 그때까지 평탄도 게이트가 나머지를 지탱한다.
