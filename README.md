...
- **녹화**: 라이브 컬러 H.264(`● Rec`) — 아래 참조
- **스냅샷** 저장(PNG), 카메라별 · 전체 Start/Stop, 녹화 중 정지 시 확인
- **화면 맞춤(Fit)**으로 잘림 없이 전체 표시 + 휠 **줌** / 드래그 **팬**, **1:1**,
  **더블클릭 전체화면**(ESC로 복귀)
- **밝기 미터**: 채널별 Rec.601 luma와 포화 화소 비율을 창 하단 그래프에 상시 실시간 표시(토글 없음)
...
| | `● Rec` (라이브 컬러) |
|---|---|
| 경로 | MIL → 메모리 → ffmpeg **파이프** |
| 부하 시 | **프레임 드롭**(라이브 뷰 우선) |
| 해상도 프리셋 | 적용됨(Original/1080p/720p) |

출력 폴더와 해상도는 `%LocalAppData%\MatroxFrameGrabber\settings.json`에 자동 저장됩니다.
