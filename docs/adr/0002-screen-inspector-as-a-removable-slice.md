...
## 카메라별 수치 표시 — 구조는 이미 있고, 비용은 실측했다

cs는 프레임 위에 박스와 HUD(`Green: N`, `Infer: N ms`)를 `Cv2.Rectangle` / `Cv2.PutText`로
**그려 넣는다.** 우리는 그렇게 하면 안 된다 — 그 픽셀이 그대로 녹화·증거·정지화면에 남는다.
증거가 비트 정확이라는 것은 실측으로 확인한 성질이고(프레임 5000이 같은 프레임 PNG와 바이트
단위로 동일), 오버레이를 버퍼에 그리는 순간 그게 깨진다.

**대신 WPF로 디스플레이 위에 얹는다. 이미 그렇게 하고 있다.**

```csharp
ViewContentGrid.Children.Insert(0, _display);   // MIL 디스플레이가 0번
// 그 위 같은 Grid에 RoiEditSurface가 ROI 사각형과 핸들 8개를 얹는다
```

- `DisplayMapping`이 **취득 픽셀 → 컨트롤 좌표**를 이미 변환한다. MIL이 이미지를 가운데
  정렬하면서 `M_REAL_OFFSET_X`에 0을 보고하는 실측 함정까지 그 안에 들어 있다.
- 녹화 배너(`RecordingBannerText`)가 이미 같은 Grid에 `TextBlock`으로 떠 있다. 카메라별 수치는
  그것과 같은 물건이다.
- 갱신은 500 ms 틱 하나 위에서 돈다. 새 경로가 필요 없다.

**따라서 표시는 무리가 아니다 — 새로 만들 기계가 없다.** 판독 박스는 `RoiEditSurface`와 같은
길, 수치는 배너와 같은 길이다.

### 추론이 취득 옆에서 견디는가 — 실측 (2026-09-11)

cs 문서의 2.77 ms는 **유휴 머신** 값이다. 우리에게 필요한 것은 3채널이 120 fps로 도는 옆에서의
숫자라, 별도 하니스로 직접 쟀다. 같은 머신, `green_detector_192_fp32.onnx`,
입력은 그래프가 말하는 대로 `images uint8 [1,256,320,3]`, ORT 스핀은 끔.

**유휴 — cs 값이 재현된다.**

| IntraOpNumThreads | 중앙값 | p99 |
|---|---|---|
| 1 | 8.722 ms | 9.414 |
| 2 | 4.976 | 5.719 |
| 4 | 3.352 | 4.124 |
| 8 | **2.696** | 3.685 |
| 0 (ORT 기본) | **2.707** | 3.690 |

기본값이 8과 같다. **cs가 "모델마다 최적 스레드 수를 다시 재라"고 한 것은 맞지만, 이 모델에서
기본값을 건드릴 이유는 없다.**

**취득 3채널 120 fps 옆에서 — 유실 0.**

| 부하 | 추론 중앙값 | p99 | max | 유실 (ch0/1/2) | 읽기 평균 |
|---|---|---|---|---|---|
| 없음 (기준) | — | — | — | 0 / 0 / 0 | 334 / 375 / 329 µs |
| 순찰 1.5 Hz (3채널 × 0.5 Hz) | **4.602 ms** | 6.940 | 6.940 | **0 / 0 / 0** | 334 / 375 / 329 µs |
| 연속 212회/초 (duty 100%) | **4.711 ms** | 6.786 | 8.888 | **0 / 0 / 0** | 421 / 617 / 568 µs |

**코어를 다 쓰며 쉬지 않고 추론해도 프레임을 잃지 않는다.** 읽기 평균이 334 → 421~617 µs로
오르지만 프레임 주기 8333 µs에 한참 못 미친다.

### 왜 증거 덤프는 프레임을 잃었는데 추론은 안 잃는가

이게 이 실측에서 제일 쓸모 있는 결론이다. **우리 취득은 CPU가 아니라 대역폭에 묶여 있다.**

- 증거 덤프는 링 버퍼에서 660 MB를 읽어 디스크로 민다 — 취득 DMA와 **같은 경로**를 쓴다.
  그래서 직렬화 전에는 레이트가 120 → 57~89 fps로 무너졌다.
- 추론은 246 KB짜리 텐서 위의 **계산**이다. 캐시에 들어가고 메모리 버스를 거의 건드리지 않는다.
  코어를 다 써도 취득이 쓰는 자원과 겹치지 않는다.

**CPU를 태우는 일은 거의 공짜이고, 메모리·디스크를 태우는 일이 비싸다.** 이 시스템에서 무엇을
추가할지 판단할 때의 기준이다.

### 그래서 "매 프레임 불가"의 이유가 바뀐다

위의 예산 표는 3채널 매 프레임을 `3 × 2.77 = 8.31 ms = 예산 100%`로 기각했다. **결론은 맞지만
이유가 틀렸다.** 추론은 취득 예산 안에서 돌지 않는다 — 다른 코어에서 돈다.

진짜 이유는 **처리량**이다. 실측 상한이 **212회/초**이고 3채널 매 프레임은 **360회/초**가
필요하다. 못 한다.

그 대신 상한이 알려졌다: **채널당 약 70 fps까지는 유실 없이 판독할 수 있다.** 순찰이 쓰려는
0.5 Hz는 그 용량의 0.7%다. 필요하면 순찰을 훨씬 촘촘하게 돌려도 된다.

### 이 실측이 말하지 않는 것

- **별도 프로세스로 쟀다.** 인프로세스면 GC와 틱 스레드까지 공유한다. 네이티브 ORT 스레드가
  지배적이라 크게 다르지 않을 것으로 보지만, 확인한 것은 아니다.
- **UI 응답성은 재지 않았다.** 로그로는 알 수 없다.
- **이 모델에 대한 값이다.** 더 큰 모델은 계산도 메모리도 함께 커진다.
- 하니스는 `$CLAUDE_JOB_DIR`에 있고 저장소에 넣지 않았다. 재현하려면 ORT 1.29.0과
  `--threads/--hz/--burst/--start-after`를 받는 20줄짜리 루프면 된다.
